...
//    static double xmin = 0, xmax = 0., xsml = 0., dxrel = 0.;
	...
	FIXME for threads ! */
//    if (ngam == 0) {
//	MT(ngam) = chebyshev_init(gamcs, 42, DBL_EPSILON/20);/*was .1*d1mach(3)*/
//	gammalims(&MT(xmin), &MT(xmax));/*-> ./gammalims.c */
//	MT(xsml) = exp(fmax2(log(DBL_MIN), -log(DBL_MAX)) + 0.01);
//	/*   = exp(.01)*DBL_MIN = 2.247e-308 for IEEE */
//	MT(dxrel) = sqrt(1/DBL_EPSILON);/*was (1/d1mach(4)) */
 //   }
#else
...
# define dxrel 67108864.
	...
	--n;
	value = chebyshev_eval(y * 2 - 1, (double*)gamcs, ngam) + .9375;
	if (n == 0)
	...
	if (n < 0) {
	    ...
	    if (x < -0.5 && fabs(x - (int)(x - 0.5) / x) < dxrel) {
			printf("gammafn: precision error\n");
	    }

	    /* The argument is so close to 0 that the result would overflow. */
	    if (y < xsml) {
			printf("gammafn: range error\n");
	...
	if(y <= 50 && y == (int)y) { /* compute (n - 1)! */
	    ...
	    value = exp((y - 0.5) * log(y) - y + M_LN_SQRT_2PI +
			((2*y == (int)2*y)? stirlerr(state, y) : lgammacor(y)));
	}
	if (x > 0)
	    return value;
